...
{
  const { Surgery } = useContext(MainContext);
  const navigate = useNavigate();

  const emptyRow = { SurgeryName: "", eye: "", comment: ""};
  ...
  setItems([{
    SurgeryName: Surgery[0]?.SurgeryName || "",
    eye: Surgery[0]?.eye || "",
    comment: Surgery[0]?.comment || "",
    id:Surgery[0]?.comment || ""
  }]);
